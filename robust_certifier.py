...
def build_parser():
    ...
    p.add_argument("--numpy-exec", dest="numpy_exec", action="store_true",
                   help="hybrid modes only: measure the centre deviation (and, for "
                        "hybrid-meas, the per-layer activation norms) from the "
                        "IEEE-754-compliant numpy execution (compliant_forward) "
                        "instead of the Keras/TF one. Use when the deployed "
                        "execution being certified is the compliant numpy forward "
                        "pass (whose y1 the input JSON must then carry)")
...
def main():
    ...
    numpy_exec = args.numpy_exec
    if numpy_exec and not (hybrid_only_mode or hybrid_meas_mode):
        print("Warning: --numpy-exec only affects the hybrid modes' measurement "
              "passes; it is a no-op for --mode standard.")
    ...
    # Both hybrid modes measure a center deviation D^hybrid from the ACTUAL
    # deployed execution (high-precision fp64 reference vs the target-format
    # model), with biases when present. This makes the measured term sound by
    # construction (same execution that produces the deployed logits) and
    # bias-aware. hybrid-meas additionally measures the per-layer activation
    # norms for the E_ball radii. The deployed execution is Keras/TF by default;
    # with --numpy-exec it is the IEEE-754-compliant numpy forward pass
    # (compliant_forward), matching input JSONs whose y1 was generated by it.
    keras_model_hi = None
    keras_model_fp = None
    keras_fp_policy = None
    if hybrid_only_mode or hybrid_meas_mode:
        if fmt.name not in ("float16", "float32", "float64"):
            sys.exit(f"hybrid-mode forward pass not supported for {fmt.name} "
                     f"(only float16/float32/float64)")
        keras_fp_policy = fmt.name
        if numpy_exec:
            from compliant_forward import (prepare as build_activation_model,
                                           forward_activations as _np_forward_activations)
            # same call shape as keras_forward.forward_activations; the format
            # is baked into the prepared model, so the policy arg is ignored
            def forward_activations(model, x, policy):
                return _np_forward_activations(model, x)
            backend = "compliant-numpy"
        else:
            from keras_forward import build_activation_model, forward_activations
            backend = "Keras"
        print(f"Building {backend} forward-pass models (fp64 ref + {keras_fp_policy} target)"
