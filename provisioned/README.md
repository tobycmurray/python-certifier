...
Each `tests/inputs_*` bundle carries BOTH `y1` variants of its certification
inputs: `inputs.json` (Keras/TF execution) and `inputs_numpy.json` (the
IEEE-754-compliant numpy execution that `run_tests.sh NUMPY_EXEC=1` -- the
paper's evaluation -- consumes; generated by
`models/provision_numpy_all_inputs.sh`), plus the `inputs_first100_numpy.json`
slices for the EMNIST dirs. The image-dataset numpy variants are committed in
git directly (their `tests/all_*_test_inputs/` dirs are tracked).
