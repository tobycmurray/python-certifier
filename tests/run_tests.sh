...
run_test() {
  ...
  python "$CERTIFIER" --mode "$mode" --json-output "$json_output" $bias_flag "$format" "$nn_file" "$gram" "$cex_file" ${ref_results_file:+"$ref_results_file"} > .log 2>&1 || (cat .log; die "Couldn't run python certifier")
